...
// In-memory knowledge store (shared with record-knowledge)
...
export async function get_knowledge(params: {
  ...
  const { sourceFramework, targetFramework, query, limit } = params;
  ...
  try {
    ...
    if (sourceFramework) {
      results = results.filter(
        (k) => k.sourceFramework.toLowerCase().includes(sourceFramework.toLowerCase())
      );
    }

    if (targetFramework) {
      results = results.filter(
        (k) => k.targetFramework.toLowerCase().includes(targetFramework.toLowerCase())
      );
    }
    ...
    if (query) {
      ...
      results = results.filter(
        (k) =>
          ...
          k.issuesFound.some((i: string) => i.toLowerCase().includes(q)) ||
          k.successfulMappings.some(
            (m: { source: string; target: string; confidence: number }) =>
              m.source.toLowerCase().includes(q) ||
              m.target.toLowerCase().includes(q)
          ) ||
          k.failedMappings.some(
            (m: { source: string; reason: string }) =>
              m.source.toLowerCase().includes(q) ||
              m.reason.toLowerCase().includes(q)
          ) ||
          k.projectPath.toLowerCase().includes(q) ||
          k.sourceFramework.toLowerCase().includes(q)
      );
    }
    ...
    // Apply limit
    const finalResult = limit ? results.slice(0, limit) : results;
